...
TMT is an **isobaric labeling** strategy—meaning all tags have identical total mass. Peptides from multiple samples are chemically tagged, **combined**, and measured together. Identification comes from peptide fragments, while quantification comes from **low‑m/z reporter ions** released from the tags during MS2.
...
The multiplexed peptide mixture is injected onto a nano‑flow C18 column that is bonded directly to the mass spectrometer inlet (Figure 7).

| <img src="images/mass_spectrometer.png" width="50%"> |
| :---- |
| Figure 7: Mass spectrometer with an HPLC column. |

During the chromatographic gradient peptides interact with the stationary phase according to their hydrophobicity and elute at characteristic retention times, producing a series of Gaussian‑like peaks (Figure 8).
...
When a particular peptide reaches the electrospray tip it is ionised, and the mass spectrometer records a full‑scan spectrum of every ion present at that retention time. This first survey scan is called **MS1** (Figure 9).
...
- **Reporter ions (low‑mass side)**: reveal the relative abundance of that peptide across all multiplexed samples. Cleavage of a single, labile bond (see dashed line in Figure 4) liberates the reporter group whose mass differs by exact integer increments (126–131 Da in TMT 6‑plex, 126–134 Da in TMTpro 18‑plex). Quantifying the area of each reporter ion yields a direct measurement of the peptide's abundance in every replicate.
...
Consequently an MS2 spectrum is registered (Figure 13). Reporter ions for the blue channels are reduced to low background counts rather than being entirely absent, which means no computational imputation is required downstream.
...
- **Consequence (ratio compression)**: the observed fold change is reduced compared to the true biological difference (see green signal in the TMT region in Figure 13). This compression is often on the order of **~10–30%**.
